...
async def add_new_task_second(msg : Message, state : FSMContext):
  if len(msg.text) < 30:
    ...
    task = msg.text
    todolist.add_values(column=('user_id', 'username', 'task'), 
                        values=(user_id, username, task))
...
async def edit_note_second(msg : Message, state : FSMContext):
  ...
  user_id = str(msg.from_user.id)
  username = msg.from_user.username
  users.update_value(column='note', value=new_note, id=user_id, no_copy=True,
                     column_for_add=('user_id', 'username'),
                     values_for_new=(user_id, username))
